...
public class ModelDaoImpl implements ModelDao {
	...
	public void isBan(Employee emp) {
		...
		try {
			...
			Connection connection=DbUtility.getConnection();
			PreparedStatement sequenceStatement = connection.prepareStatement("select tentative_return_date,item_type,item_id from product_borrow where user_id=? and is_return=false");
			sequenceStatement.setInt(1,emp.getUser_id());
			ResultSet rs = sequenceStatement.executeQuery();
			
			//This needs to be String type
			Timestamp tent_date_return=null;
			Date today=new Date();
			Timestamp curr_date=new Timestamp(today.getTime());
			...
			if(curr_date.after(tent_date_return) && !is_return)
			{
				...
				lendingStatement.setInt(1,emp.getUser_id());
				lendingStatement.executeUpdate();
				
				lendingStatement = connection.prepareStatement("select ban_days,fine from asset_type where type=?");
				...
				if(rs1.next()) {
					int lending=rs1.getInt(1);
					double total_fine=rs1.getFloat(2);
					...
					total_fine*=diff;
					lendingStatement=connection.prepareStatement("update product_borrow set current_fine=? where user_id=? and asset_id=?");
					lendingStatement.setInt(1,emp.getUser_id());
